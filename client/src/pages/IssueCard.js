...
function IssueCard( { issues, setIssues } ) {
    ...
    const [userIssues, setUserIssues] = useState(user.issues)
    ...
    function handleDelete(id) {
        ...
        .then(r => {
          if (r.ok) {
            ...
            const userUpdatedIssues = userIssues.filter(issue => foundIssue.id !== issue.id)
            setIssues(newIssues)
            setUser({...user, movies: userUpdatedIssues})
          }
        })
      }
    ...
    return (
        <Wrapper>
            ...
            <Box>
                {hasUserCommented(foundIssue)
                    ? null 
                    :
                    <NewComment handleAddComment={handleAddComment} issueId={issue.id} userId={user.id} user={user} setUser={setUser} issues={issues}></NewComment>
                }

                {issue.issues_with_comments?.map((comment) => (
                    <Box key={comment.comment_id}>
                        {comment.content}<br></br><br></br>
                        by: <em>{comment.username}</em><br></br>
                        {comment.username === user.username ?
                        <>
                            <button onClick={() => handleDelete(comment.comment_id)}>Delete</button>     
                            <button onClick={() => setToggleNewReview(toggle => !toggle)}>Edit your Comment</button>
                            {toggleNewReview ? 
                            <form>
